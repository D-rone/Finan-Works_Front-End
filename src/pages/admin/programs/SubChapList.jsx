...
import add from "../../../assets/addEmployee.svg";
import AddSubChapForm from "./AddSubChapterForm";
...
export default function SubChaptersList() {
  ...
  let checkHandle = (e) => {
    ...
    if (name === "all") {
      ...
      tmp = subChapters.map((subChp) =>
        subChp.id === name ? { ...subChp, isHidden: checked } : subChp
      );
  ...
  let updateSubChaptersStatus = async () => {
    ...
    if (updated.length > 0) {
      try {
        ...
        const response = await axios.patch(
          "http://localhost:5000/api/v1/sousChapitre/admin/statusUpdate/",
          updated.map((subCh) => {
            return {
              _id: subCh.id,
              status: subCh.isHidden ? "desactive" : "active",
            };
          }),
          {
            withCredentials: true,
          ...
          setSubChapters((prev) => {
            {
              return prev.map((item) => {
                return {
                  ...item,
                  status: item.isHidden ? "desactive" : "active",
                };
              });
            }
          });
          toast.success("Subchapters status have been updated", {
            autoClose: 2000,
      ...
      toast.error("There is no updated subchapter", {
        autoClose: 2000,
        position: toast.POSITION.BOTTOM_RIGHT,
      });
    }
  };
  let [addSubChapForm, setAddSubChapForm] = useState(false);

  let toggleAddSubChapterForm = () => {
    setAddSubChapForm((prev) => !prev);
  };
  ...
  return (
    <section id="mainSection">
      ...
      {addSubChapForm ? (
        <AddSubChapForm
          toggle={toggleAddSubChapterForm}
          update={updateSubChapters}
          chapterId={chapterId}
        />
      ) : (
        ""
      ...
      <div className="pageList" id="subChaptersList">
            ...
            <table id="listTable">
              <thead>
                <tr>
                  ...
                  <th style={{ height: "0px" }} width="34%"></th>
                  <th style={{ height: "0px" }} width="26%"></th>
                  <th style={{ height: "0px" }} width="25.7%"></th>
                </tr>
              </thead>
              {loading ? (
                <tbody>
        ...
        <footer>
          <div className="footerOptions">
            ...
            {loading2 ? (
              <div className="loader" id="hideLoading">
                <div className="one">
                  <svg
                    width="162"
                    height="148"
            ...
            <button id="addEmployee" onClick={toggleAddSubChapterForm}>
              <img src={add} alt="" />
              Add
            </button>
          </div>
