...
export let DrawForm = ({ id, data, toggle }) => {
  ...
  let draw = async () => {
    try {
      setLoading(true);
      if (drawCount > 0 && drawCount <= data?.emplInscrit?.length) {
        function getRandomElementsFromArray(array, n) {
          if (n > array.length) {
            throw new Error(
              "Invalid argument: n cannot be greater than the array length"
            );
          }
          ...
          return copyArray.slice(0, n);
        }

        let randomWinners = getRandomElementsFromArray(
          data?.emplInscrit,
          drawCount
        );
        setWinners(randomWinners);
        let body = randomWinners.map((winner) => {
          return { idEmpAdmis: winner?.emplInscrits?._id };
        });
        const response = await axios.post(
          "http://localhost:5000/api/v1/annonce/admin/admisAnnonce/" + id,
          body,
          {
        ...
        toast.error(
          "Count should be lower than number of subscribed employees",
          {
            autoClose: 2000,
            position: toast.POSITION.BOTTOM_RIGHT,
          }
  ...
  return (
    <>
      <div id="shade" className="shade2">
        <div id="rejectPopup">
          <h3 id="rejectRequestHeader">DRAW</h3>
          <div>
            <input
              id="drawCount"
              type="number"
          ...
          {winners.length > 0 ? (
            <>
              <h5>Winners:</h5>
              {winners.map((winner) => (
                <div>{winner?.emplInscrits?.name}</div>
              ))}
            </>
          ) : (
            ""
          ...
          <div id="rejectPopUpBtns">
            <button onClick={draw} id="Popup_confirm">
              Confirm
            </button>
            <button onClick={toggle} id="Popup_cancel">
              Cancel
            </button>
          </div>
        </div>
...
export default function AnnounceData({ data, handleAnnounceClick }) {
  let [drawForm, setDrawForm] = useState(false);

  let [annEmpls, setAnnEmpls] = useState({ emplAdmis: [], emplInscrit: [] });
  let getAnnEmpls = async () => {
    try {
      const response = await axios.get(
        "http://localhost:5000/api/v1/annonce/emp/getSingleAnnonce/" + data.id,
        {
          withCredentials: true,
        }
      );
      let annData = response?.data;
      setAnnEmpls({
        emplAdmis: annData?.emplAdmis || [],
        emplInscrit: annData?.empInscrits || [],
      });
    } catch (err) {
      console.log(err);
    }
  ...
  return (
    <div id="shade">
      <div id="popUpContainer">
        ...
        {drawForm ? (
          <DrawForm data={annEmpls} toggle={toggleDrawForm} id={data.id} />
        ) : (
          ""
        )}

        <h2>Announce Information</h2>
        <div className="chapitre_information_container_other_announce">
          <div className="new_chapitre_information_other_announce">
            <div className="new_chapitre_information_titles_other_announce">
              <div>Description :</div>
            ...
            <div className="new_chapitre_information_formaulaire_other_announce">
              <div>{data.description}</div>
              <div>{data.createdAt.toGMTString()}</div>

              <div>{data.updatedAt.toGMTString()}</div>
            </div>
          </div>
        </div>

        <div className="container_lists">
          <div className="Employees_inscrits">
            <div className="Employees_inscrits_title">
              Subscribed Employees :{" "}
            </div>
            <div className="Employees_inscrits_data">
              {annEmpls.emplInscrit.length > 0
                ? annEmpls.emplInscrit?.map((item) => (
                    <div key={item?.emplInscrits?._id}>
                      {item?.emplInscrits?.name}
                    </div>
                  ))
                : "No subscription yet"}
            </div>
          ...
          <div className="Employees_admis">
            <div className="Employees_admis_title"> Admitted Employees :</div>
            <div className="Employees_admis_data">
              {annEmpls.emplAdmis.length > 0
                ? annEmpls.emplAdmis?.map((item) => (
                    <div key={item?.empAdmis?._id}>{item?.empAdmis?.name}</div>
                  ))
                : "No winner yet"}
            </div>
          </div>
        </div>
        <button id="drawBtn" onClick={toggleDrawForm}>
          DRAW
        </button>
      </div>
    </div>
